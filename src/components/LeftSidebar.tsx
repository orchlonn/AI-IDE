...
export default function LeftSidebar({
  ...
  if (!isOpen) {
    return (
      <button
        type="button"
        onClick={onToggle}
        className="glass absolute left-3 top-16 z-10 rounded-lg p-2 text-[var(--muted)] transition-colors hover:text-[var(--foreground)]"
  ...
  return (
    <aside
      className="relative flex shrink-0 flex-col border-r border-[var(--border)] bg-[var(--sidebar-bg)]"
      style={{ width }}
    >
      <div className="flex h-9 shrink-0 items-center justify-between border-b border-[var(--border)] px-3">
        <span className="text-[10.5px] font-semibold uppercase tracking-[0.08em] text-[var(--muted)]">
          Explorer
        </span>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <div className="flex items-center gap-2 border-b border-[var(--border)] px-3 py-2.5">
          <div
            className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md text-[var(--accent)]"
        ...
        <div className="flex-1 overflow-y-auto overflow-x-auto px-1.5 py-1.5">
          {fileTree.map((node) => (
            <FileTreeItem
              key={node.name}
              node={node}
      ...
      <div
        className="absolute top-0 right-0 z-10 h-full w-1 cursor-col-resize transition-colors hover:bg-[var(--accent)]/40"
